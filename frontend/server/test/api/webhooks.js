...
describe('api/webhooks', function () {
  it('does not fire if not enabled', async function () {
    ...
    await utils.post('admin', '/selekt/api/users', {
  ...
  it('userCreated', async function () {
    ...
    const user = await utils.post('admin', '/selekt/api/users', {
      email: 'user1@test.com',
      name: 'user1',
      role: 'editor',
    });
    ...
    assert.equal(hookServer.responses[0].headers['selekt-secret'], 'secret');
  ...
  it('queryCreated', async function () {
    ...
    assert.equal(hookServer.responses[0].headers['selekt-secret'], '');

    const body1 = hookServer.responses[0].body;
    assert.equal(body1.action, 'query_created');
    assert.equal(body1.selektUrl, '');
