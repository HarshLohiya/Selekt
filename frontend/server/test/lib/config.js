...
describe('lib/config/from-default', function () {
  it('provides expected values', function () {
    ...
    assert(conf.dbPath !== '$HOME/selekt/db', 'dbPath should change');
  });
});

describe('lib/config/from-env', function () {
  it('provides expected values', function () {
    const conf = getFromEnv({ SELEKT_PORT: 8000 });
...
describe('lib/config', function () {
  ...
  it('Errors for old env var', function () {
    const config = new Config({}, { SELEKT_DEBUG: 'true' });
    const validations = config.getValidations();
    assert(validations.errors);
    const found = validations.errors.find(
      (error) =>
        error.includes('SELEKT_DEBUG') && error.includes('NOT RECOGNIZED')
    );
    assert(found, 'has error about old key');
  });

  it('Warns for unknown env var', function () {
    const config = new Config({}, { SELEKT_UNKNOWN_VAR: 'true' });
    const validations = config.getValidations();
    assert(validations.warnings);
    const found = validations.warnings.find(
      (warning) =>
        warning.includes('SELEKT_UNKNOWN_VAR') &&
  ...
  it('Errors env connection missing name', function () {
    const config = new Config(
      {},
      { SELEKT_CONNECTIONS__test__driver: 'postgres' }
    );
    const validations = config.getValidations();
    assert(validations.errors);
    const found = validations.errors.find((error) =>
      error.includes('SELEKT_CONNECTIONS__test__name missing')
    );
  ...
  it('Errors env connection missing driver', function () {
    const config = new Config(
      {},
      { SELEKT_CONNECTIONS__test__name: 'My test' }
    );
    const validations = config.getValidations();
    assert(validations.errors);
    const found = validations.errors.find((error) =>
      error.includes('SELEKT_CONNECTIONS__test__driver missing')
    );
    assert(found, 'has error');
  });
  ...
  it('Errors env connection invalid driver', function () {
    const config = new Config(
      ...
      {
        SELEKT_CONNECTIONS__test__name: 'My test',
        SELEKT_CONNECTIONS__test__driver: 'foo',
      }
    );
    const validations = config.getValidations();
    assert(validations.errors);
    const found = validations.errors.find((error) =>
      error.includes(
        'Environment config SELEKT_CONNECTIONS__test__driver invalid. "foo" not a supported driver.'
      )
    );
    assert(found, 'has error');
  });
  ...
  it('Errors env connection invalid driver field', function () {
    const config = new Config(
      {},
      {
        SELEKT_CONNECTIONS__test__name: 'My test',
        SELEKT_CONNECTIONS__test__driver: 'postgres',
        SELEKT_CONNECTIONS__test__wrongField: 'localhost',
      }
    );
    const validations = config.getValidations();
    assert(validations.errors);
    const found = validations.errors.find((error) =>
      error.includes(
        'Environment config SELEKT_CONNECTIONS__test__wrongField invalid. "wrongField" not a known field for postgres.'
      )
    );
    assert(found, 'has error');
  });
