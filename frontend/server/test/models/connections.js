...
describe('config.getConnections', function () {
  ...
  it('skips partial connections', function () {
    const cs = utils.config.getConnections({
      SELEKT_CONNECTIONS__abc__driver: 'postgres',
    });
    assert(Array.isArray(cs));
  ...
  it('parses connection properly', function () {
    const cs = utils.config.getConnections({
      SELEKT_CONNECTIONS__abc__driver: 'postgres',
      SELEKT_CONNECTIONS__abc__name: 'env-postgres',
      SELEKT_CONNECTIONS__abc__host: 'localhost',
      SELEKT_CONNECTIONS__abc__port: '5432',
      SELEKT_CONNECTIONS__abc__postgresSsl: 'true',
    });
    assert(Array.isArray(cs));
  ...
  it('includes env connection in all connections', async function () {
    process.env.SELEKT_CONNECTIONS__abc__driver = 'postgres';
    process.env.SELEKT_CONNECTIONS__abc__name = 'env-postgres';
    process.env.SELEKT_CONNECTIONS__abc__host = 'localhost';
    process.env.SELEKT_CONNECTIONS__abc__port = '5432';
    process.env.SELEKT_CONNECTIONS__abc__postgresSsl = 'true';
    ...
    delete process.env.SELEKT_CONNECTIONS__abc__driver;
    delete process.env.SELEKT_CONNECTIONS__abc__name;
    delete process.env.SELEKT_CONNECTIONS__abc__host;
    delete process.env.SELEKT_CONNECTIONS__abc__port;
    delete process.env.SELEKT_CONNECTIONS__abc__postgresSsl;
  });
