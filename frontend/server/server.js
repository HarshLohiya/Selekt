...
const helpText = `
  selekt version ${packageJson.version}
  ...
  # Start selekt using config file:
...
if (argv.version || cliHas('version')) {
  ...
  console.log('selekt version %s', packageJson.version);
  process.exit(0);
}
...
const configFilePath = argv.config || process.env.SELEKT_CONFIG;
...
// More info
//
// https://github.com/selekt/selekt/pull/185
...
function detectPortOrSystemd(port) {
  if (systemdSocket) {
    ...
    // We just crab the first socket from fd 3 since selekt binds only one
    // port.
...
async function startServer() {
  ...
  // Check to ensure selekt is either v0 (not yet initialized) or v5 or later
  ...
  if (incompatibleDbVersion) {
    appLog.error(
      'selekt database not compatible with this version of selekt. Migrate to version 5 prior to running version 6 or later.'
    );
    process.exit(1);
  }
  ...
  if (!isUpToDate && !runMigrations) {
    appLog.error(
      'selekt schema not up to date. Turn on automigration or use --migrate'
    );
    process.exit(1);
  }
  ...
  if (keyPath && certPath) {
    ...
    server = https
      ...
      .listen(_port, ip, function () {
        ...
        appLog.info('Welcome to selekt!. Visit %s to get started', url);
      });
  } else {
    // http only
    const _port = await detectPortOrSystemd(port);
...
startServer().catch((error) => {
  appLog.error(error, 'Error starting selekt');
  process.exit(1);
});
