...
const dropTable = 'DROP TABLE IF EXISTS SELEKT_test;';
const createTable = 'CREATE TABLE SELEKT_test (id INTEGER, name TEXT );'; // NOTE test(s) will fail if table already exists, expect empty database
const insert1 = "INSERT INTO SELEKT_test (id, name) VALUES (1, 'one');";
const insert2 = "INSERT INTO SELEKT_test (id, name) VALUES (2, 'two');";
const insert3 = "INSERT INTO SELEKT_test (id, name) VALUES (3, 'three');";
...
describe('drivers/unixodbc', function () {
  ...
  it('runQuery under limit', async function () {
    const results = await unixodbc.runQuery(
      'SELECT * FROM SELEKT_test WHERE id = 1;',
      connection
    );
  ...
  it('Client handles multiple runQuery calls', async function () {
    ...
    const results1 = await client.runQuery('SELECT * FROM SELEKT_test');
    assert.equal(results1.incomplete, false);
    assert.equal(results1.rows.length, 3);
    const results2 = await client.runQuery('SELECT * FROM SELEKT_test');
