...
class Webhooks {
  /**
   *
   * @param {import('./config/index.js')} config
   * @param {import('../models/index.js')} models
   * @param {import('./app-log.js')} appLog
   */
  ...
  async send(hookName, url, body) {
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          action: hookName,
          selektUrl: this.selektUrl(),
          ...body,
        }),
        headers: {
          'Content-Type': 'application/json',
          'selekt-Secret': config.get('webhookSecret'),
        },
      });
