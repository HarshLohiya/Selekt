...
const oldConfigFilePath = path.join(userHome, '.selektrc');
const oldDefaultDbPath = path.join(userHome, 'selekt/db');
...
if (fs.existsSync(oldConfigFilePath)) {
  ...
  Use --config at command line or set environment variable SELEKT_CONFIG
  `;
}
...
export function isConnectionEnv(key = '') {
  return key.startsWith('SELEKT_CONNECTIONS__');
}
 ...
 * SELEKT_CONNECTIONS__<connectionId>__<connectionFieldName>
 *
 * Example: SELEKT_CONNECTIONS__ab123__sqlserverEncrypt=""
 *
 * @param {object} env
 */
export function parseConnectionsFromEnv(env) {
  const connectionsMap = Object.keys(env)
    .filter((key) => key.startsWith('SELEKT_CONNECTIONS__'))
